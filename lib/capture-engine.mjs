 ...
 *   1. Wait for 'domcontentloaded' (reliable, works on all pages)
 *   2. Optionally wait for 'networkidle' with a short timeout (best-effort)
 */
async function navigateAndSettle(page, url, delay) {
  await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 30000 });

  try {
    await page.waitForLoadState('networkidle', { timeout: 5000 });
  } catch {
    // Expected on analytics-heavy pages (Shopify, etc.) — continue anyway
  }
  ...
  for (const browserName of browserNames) {
    ...
    for (const deviceName of deviceNames) {
      ...
      const result = await captureWithRetry(async () => {
        ...
        await navigateAndSettle(page, url, delay);
